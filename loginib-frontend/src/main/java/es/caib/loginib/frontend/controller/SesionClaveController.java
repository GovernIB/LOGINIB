...
public final class SesionClaveController {
  ...
  /** Log. */
  private final Logger log = LoggerFactory.getLogger(SesionClaveController.class);
  ...
  public ModelAndView iniciarSesionClaveExterna(@RequestParam("idSesion") final String idSesion) {
    ...
    if (claveService.isAccesoClaveSimulado()) {
      ...
      return new ModelAndView("simularClave", "simularClave", simularClave);
    }

    // Generamos request SAML
    final PeticionClave peticionClave = claveService.generarPeticionClave(idSesion);
  ...
  public ModelAndView iniciarLogoutSesionClaveExterna(@RequestParam("idSesion") final String idSesion) {
    ...
    // Generamos request SAML
    PeticionClaveLogout peticionClave = claveService.generarPeticionLogout(idSesion);
  ...
  public ModelAndView retornoClaveExterna(@PathVariable("idSesion") final String idSesion,
   ...
   *         login en Clave
   */
  @RequestMapping(value = "/logoutClave/{idSesion}.html", method = RequestMethod.POST)
  public ModelAndView retornoLogoutClaveExterna(@PathVariable("idSesion") final String idSesion,
      @RequestParam("samlResponseLogout") final String samlResponse) {

	  RespuestaClaveLogout resp = claveService.procesarRespuestaLogout(idSesion, samlResponse);
  ...
  public ModelAndView loginClaveSimulado(@RequestParam("idSesion") final String idSesion,
    ...
    final TicketClave ticket = claveService.simularRespuestaClave(idSesion, idp, nif, nombre,
   ...
   */
  private ModelAndView retornoClave(final String idSesion, final String samlResponse) {
    log.debug("Retorno clave: id sesion = " + idSesion);

    // Generamos ticket autenticacion
    final TicketClave ticket = claveService.procesarRespuestaClave(idSesion, samlResponse);
