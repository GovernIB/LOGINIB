...
public final class SesionClaveController {
	...
	public ModelAndView seleccionAutenticacion(@RequestParam("idSesion") final String idSesion) {
		final DatosSesion sesion = claveService.obtenerDatosSesionLogin(idSesion);
	...
	public ModelAndView redirigirLoginClave(@RequestParam("idSesion") final String idSesion) {
		...
		if (claveService.isAccesoClaveDeshabilitado()) {
			...
			return new ModelAndView("redirect:/error.html?code=" + ErrorCodes.CLAVE_DESHABILITADO.toString());
		}
		...
		final PeticionClave peticionClave = claveService.generarPeticionLoginClave(idSesion);
	...
	public ModelAndView redirigirLogoutClave(@RequestParam("idSesion") final String idSesion) {
		...
		// Generamos request SAML
		final PeticionClaveLogout peticionClave = claveService.generarPeticionLogoutClave(idSesion);
	...
	public ModelAndView error(@RequestParam("code") final String errorCode, final HttpServletRequest request) {
		...
		// En caso de existir error particular, mostramos mensaje particular
		if (request.getSession().getAttribute(ERROR_ID_SESION) != null) {
			final String idSesion = (String) request.getSession().getAttribute(ERROR_ID_SESION);
			final DatosSesion datosSesion = this.claveService.obtenerDatosSesionLogin(idSesion);
			if (datosSesion.getUrlCallbackError() != null && !datosSesion.getUrlCallbackError().isEmpty()) {
				urlCallback = datosSesion.getUrlCallbackError();
			}
		}
		...
		return new ModelAndView(view, "respuesta", respuesta);
