...
public class DialogDefinicionVersionDesgloses extends DialogControllerBase {
	...
	/**
	 * Método que comprueba si está todo correcto. Es decir, las particulas de nombre, apellido1 y apellido2 están en nombreCompleto.
	 * @return
	 */
	private boolean isCorrecto() {
		final List<String> nombreCompleto = new LinkedList<String>(Arrays.asList(desglose.getNombreCompleto().split(" ")));
		...
		if (nombreCompleto.contains(desglose.getNombre())) {
			...
			UtilJSF.addMessageContext(TypeNivelGravedad.ERROR, UtilJSF.getLiteral("error.tipo.particulaInexistente", param));
			return false;
		}
